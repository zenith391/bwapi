...
global.fullModelSync = async function (id, noSource) {
  ...
  try {
    let metadata = JSON.parse(
      fs.readFileSync("models/" + id + "/metadata.json"),
    );
    for (const key in metadata) {
      model[key] = metadata[key];
    }
    if (noSource == undefined || noSource == null) {
      model["source_json_str"] = fs.readFileSync(
        "models/" + id + "/source.json",
        { encoding: "utf8" },
      );
    }
  ...
  if (!allModelsCacheValid) {
    ...
    fs.readdir("models", async function (err, files) {
      ...
      for (const i in files) {
        ...
        try {
          let json = JSON.parse(
            fs.readFileSync("models/" + file + "/metadata.json"),
          );
  ...
  fs.readFile(
    ...
    async function (err, data) {
      ...
      let metadata = {
        ...
        blocksworld_premium_coins_price: parseInt(
          value(req.body, "coins_price_markup"),
        ),
        blocksworld_premium_coins_price_blueprint: parseInt(
          value(req.body, "coins_price_markup"),
        ),
      ...
      if (req.files["iconSD"] && req.files["imageSD"]) {
        fs.copyFileSync(
          req.files["imageSD"][0].path,
          "images/models/" + newId + ".png",
        );
        fs.copyFileSync(
          req.files["iconSD"][0].path,
          "images/models/" + newId + "_icon.png",
        );
      }
  ...
  if (fs.existsSync("models/" + id)) {
    ...
    if (metadata["author_id"] == user.id) {
      ...
      if (req.body["blocks_inventory_str"]) {
        metadata["blocks_inventory_str"] = value(
          req.body,
          "blocks_inventory_str",
        );
      }
      if (req.body["source_equality_checksum"]) {
        metadata["source_equality_checksum"] = value(
          req.body,
          "source_equality_checksum",
        );
      }
      if (req.body["coins_price_markup"]) {
        metadata["coins_price"] = parseInt(
          value(req.body, "coins_price_markup"),
        );
        metadata["coins_price_blueprint"] = parseInt(
          value(req.body, "coins_price_markup"),
        );
        metadata["coins_price_markup"] = parseInt(
          value(req.body, "coins_price_markup"),
        );
        metadata["blocksworld_premium_coins_price"] = parseInt(
          value(req.body, "coins_price_markup"),
        );
        metadata["blocksworld_premium_coins_price_blueprint"] = parseInt(
          value(req.body, "coins_price_markup"),
        );
      }
      ...
      fs.writeFile("models/" + id + "/metadata.json", metaStr, function (err) {
        if (err != null) console.log("file error for model update: " + err);
        res.status(200).json({
          user_model: metadata,
        });
      });
    ...
    for (const metadata of models) {
      ...
      if (metadata["publication_status"] == 2 && cond) {
        ...
        if (
          ...
          model["model_category_id"] != categoryId
        ) {
          continue;
        }
        model = await processUserWorld(model);
...
export function run(app) {
  ...
  app.get(
    "/api/v1/current_user/purchased_u2u_models",

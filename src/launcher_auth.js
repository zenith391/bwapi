...
export async function loginToAccount(username, password) {
  if (fs.existsSync(ACCOUNTS_PATH + username + ".json")) {
    const accountData = JSON.parse(
      fs.readFileSync(ACCOUNTS_PATH + username + ".json", { encoding: "utf8" }),
    );

    if (
      !accountData["security_version"] ||
      accountData["security_version"] === 1
    ) {
      console.log("Must BW Wool login.");
      throw new Error(
        "Please login on the Blocksworld Wool website then login on the Blocksworld Launcher.",
      );
    }
    const userId = accountData["bw_user_id"];
    const result = await bcrypt.compare(
      password,
      "$2a" + accountData.password.substring(3),
    );
    // PHP produces $2y$ althought they're actually $2a$
    ...
    if (result) {
      ...
      if (
        !accountData["bw_user_id"] ||
        isNaN(parseInt(accountData["bw_user_id"]))
      ) {
        accountData["bw_user_id"] = fs.readFileSync(
          "usersWoolLinks/" + accountData["bw_link_id"] + ".txt",
          { encoding: "utf8" },
        );
        fs.writeFileSync(
          ACCOUNTS_PATH + username + ".json",
    ...
    throw new Error(
      "Invalid username or password. Do you have an account? If no, click the Register button",
    );
  }
}
...
async function account_login(req, res) {
  ...
  try {
    const authToken = await loginToAccount(
      req.body.username,
      req.body.password,
    );
    res.status(200).json(

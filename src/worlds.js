...
global.fullWorldSync = async function (id, noSource) {
  try {
    ...
    if (noSource === undefined || noSource == null) {
      world["source_json_str"] = fs.readFileSync(
        "worlds/" + id + "/source.json",
        { encoding: "utf8" },
      );
    }
  ...
  if (!allWorldsCacheValid) {
    ...
    fs.readdir("worlds", async function (err, files) {
      ...
      for (const i in files) {
        ...
        try {
          let json = JSON.parse(
            await fs.promises.readFile("worlds/" + file + "/metadata.json"),
          );
...
async function publicationStatus(req, res) {
  ...
  if (fs.existsSync("worlds/" + id)) {
    let metadata = JSON.parse(
      fs.readFileSync("worlds/" + id + "/metadata.json", { encoding: "utf8" }),
    );
    if (metadata["author_id"] == userId) {
      if (req.body.is_published == "true") {
        if (
          profanityRegex.test(metadata.title) ||
          profanityRegex.test(metadata.description)
        ) {
          res.status(400).json({
            error: 400,
...
function likeStatus(req, res) {
  ...
  const likedWorlds = JSON.parse(
    fs.readFileSync("users/" + userId + "/liked_worlds.json"),
    { encoding: "utf8" },
  );
  if (fs.existsSync("worlds/" + id)) {
    let metadata = JSON.parse(
    ...
    fs.writeFileSync(
      "users/" + userId + "/liked_worlds.json",
      JSON.stringify(likedWorlds),
    );
    res.status(200).json({});
  }
}
...
async function createWorld(req, res) {
  ...
  const data = await fs.promises.readFile("conf/new_world_id.txt", {
    encoding: "utf8",
  });
  ...
  if (req.files["screenshot_image"]) {
    fs.copyFileSync(
      req.files["screenshot_image"][0].path,
      "images/" + newId + ".png",
    );
  }
  ...
  if (fs.existsSync("worlds/" + id)) {
    ...
    if (owning) {
      if (req.body["source_json_str"]) {
        fs.writeFileSync(
          "worlds/" + id + "/source.tmp.json",
          value(req.body, "source_json_str"),
        );
        fs.copyFileSync(
          "worlds/" + id + "/source.tmp.json",
          "worlds/" + id + "/source.json",
      ...
      if (req.files["screenshot_image"]) {
        try {
          fs.copyFileSync(
            value2(req.files["screenshot_image"]).path,
            "images/" + id + ".png",
          );
        } catch (e) {
          // TODO: handle
    ...
    if (kind == "arcade") {
      ...
      worlds = worlds.map(function (world) {
        ...
        return {
          ...
          time:
            date +
            (parseInt(world["play_count"]) +
              parseInt(world["likes_count"]) * 10) *
              4000000 *
              (rate - 1),
        };
      });
    } else if (kind == "featured") {
      // Should be only 1 world, the world shown in big at top.
      const featuredWorldTxt = fs.readFileSync("conf/featured_world_id.txt", {
        encoding: "utf8",
      });
      ...
      try {
        ...
        if (cond) {
          if (
            ...
            metadata["category_ids"].indexOf(parseInt(categoryId)) == -1
          ) {
            continue;
          }
...
function playWorld(req, res) {
  ...
  if (fs.existsSync("worlds/" + id)) {
    ...
    let playedWorlds = JSON.parse(
      fs.readFileSync("users/" + userId + "/played_worlds.json"),
    );
    if (playedWorlds["worlds"].indexOf(parseInt(id)) == -1) {
      ...
      fs.writeFile(
        "users/" + userId + "/played_worlds.json",
        playStr,
        function (err) {
          if (err != null) console.log("file error for world update: " + err);
        },
      );
    }
...
export function run(app) {
  ...
  app.put(
    "/api/v1/worlds/:id/published_status",
  ...
  app.get(
    "/api/v1/current_user/world_star_rating/:id",
    async function (req, res) {
      ...
      if (fs.existsSync("worlds/" + id)) {
        let ratings = JSON.parse(
          fs.readFileSync("users/" + userId + "/world_ratings.json"),
        );
        let json = {
          average_star_rating: (await fullWorldSync(id, true))[
            "average_star_rating"
          ],
        };
  ...
  app.post("/api/v1/current_user/world_star_rating/:id", function (req, res) {
    ...
    if (fs.existsSync("worlds/" + id)) {
      ...
      if (ratings.ratings[id]) {
        meta["average_star_rating"] =
          (meta["average_star_rating"] * meta["number_of_raters"] -
            ratings.ratings[id]) /
          (meta["number_of_raters"] - 1);
        meta["number_of_raters"] -= 1;
      }
      if (
        meta["average_star_rating"] == 0 ||
        !isFinite(meta["average_star_rating"])
      ) {
        meta["average_star_rating"] = stars;
      } else {
        meta["average_star_rating"] =
          (meta["average_star_rating"] * meta["number_of_raters"] + stars) /
          (meta["number_of_raters"] + 1);
      }
      meta["number_of_raters"] += 1;

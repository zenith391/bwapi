...
export class User {
  ...
  static async create(username: string, linkType: LinkType) {
    const newId = fs.readFileSync("conf/new_account_id.txt", {
      encoding: "utf8",
    });
    fs.writeFileSync(
    ...
    if (config.EARLY_ACCESS) newUserStatus |= 4; // add "early access" flag
    let userInfo = {
      coins: 100,
      game_gems: 0,
      needs_tutorial: true,
  ...
  async getMetadata() {
    if (this._metadata === undefined) {
      let metadataJson = JSON.parse(
        fs.readFileSync("users/" + this.id + "/metadata.json", {
          encoding: "utf8",
        }),
      );
      ...
      let metadata = classTransformer.plainToClassFromExist(
        defaultMetadata,
        metadataJson,
      );
  ...
  async getFollowers() {
    if (this._followers === undefined) {
      if (!fs.existsSync("users/" + this.id + "/followers.json")) {
        fs.writeFileSync(
          "users/" + this.id + "/followers.json",
          '{"attrs_for_follow_users":{}}',
        );
      }

      const rawFollowers = JSON.parse(
        fs.readFileSync("users/" + this.id + "/followers.json", {
          encoding: "utf8",
        }),
      )["attrs_for_follow_users"];
      this._followers = [];
  ...
  async getFollowedUsers() {
    if (this._followedUsers === undefined) {
      if (!fs.existsSync("users/" + this.id + "/followed_users.json")) {
        fs.writeFileSync(
          "users/" + this.id + "/followed_users.json",
          '{"attrs_for_follow_users":{}}',
        );
      }

      try {
        const rawFollowedUsers = JSON.parse(
          fs.readFileSync("users/" + this.id + "/followed_users.json", {
            encoding: "utf8",
          }),
        )["attrs_for_follow_users"];
        this._followedUsers = [];
  ...
  async getPendingPayouts() {
    if (this._pendingPayouts === undefined) {
      this._pendingPayouts = JSON.parse(
        fs.readFileSync("users/" + this.id + "/pending_payouts.json", {
          encoding: "utf8",
        }),
      )["pending_payouts"];
    }
  ...
  async getPurchasedModels() {
    if (this._purchasedModels === undefined) {
      if (!fs.existsSync("users/" + this.id + "/purchased_u2u_models.json")) {
        fs.writeFileSync(
          "users/" + this.id + "/purchased_u2u_models.json",
          '{"u2u_models":[]}',
        );
      }
      this._purchasedModels = JSON.parse(
        fs.readFileSync("users/" + this.id + "/purchased_u2u_models.json", {
          encoding: "utf8",
        }),
      )["u2u_models"];
    }
  ...
  async getFeeds() {
    if (this._feeds === undefined) {
      if (!fs.existsSync("users/" + this.id + "/news_feed.json")) {
        fs.writeFileSync(
          "users/" + this.id + "/news_feed.json",
          '{"news_feed":[]}',
        );
      }
      this._feeds = JSON.parse(
        fs.readFileSync("users/" + this.id + "/news_feed.json", {
          encoding: "utf8",
        }),
      )["news_feed"];
    }
  ...
  async getLikedWorlds(): Promise<number[]> {
    if (this._likedWorlds === undefined) {
      if (!fs.existsSync("users/" + this.id + "/liked_worlds.json")) {
        fs.writeFileSync(
          "users/" + this.id + "/liked_worlds.json",
          '{"worlds":[]}',
        );
      }
      this._likedWorlds = JSON.parse(
        fs.readFileSync("users/" + this.id + "/liked_worlds.json", {
          encoding: "utf8",
        }),
      )["worlds"];
    }
  ...
  async follow(target: any) {
    ...
    if (fs.existsSync("users/" + targetId + "/followers.json")) {
      targetFollowers = JSON.parse(
        fs.readFileSync("users/" + targetId + "/followers.json", {
          encoding: "utf8",
        }),
      );
    } else {
  ...
  async setPendingPayouts(newValue: Payout[]) {
    ...
    fs.writeFileSync(
      "users/" + this.id + "/pending_payouts.json",
      JSON.stringify({
...
async function save_current_user_profile_world(req: any, res: any) {
  ...
  if (!fs.existsSync("users/" + userId + "/profile_world")) {
    ...
    fs.copyFileSync(
      "conf/default_profile_world.txt",
      "users/" + userId + "/profile_world/source.json",
    );
    fs.writeFileSync(
      "users/" + userId + "/profile_world/metadata.json",
      JSON.stringify({
  ...
  let meta = JSON.parse(
    fs.readFileSync("users/" + userId + "/profile_world/metadata.json", {
      encoding: "utf8",
    }),
  );
  if (req.body["source_json_str"]) {
    fs.writeFileSync(
      "users/" + userId + "/profile_world/source.json",
      value2(req.body["source_json_str"]),
    );
  }
  if (req.body["avatar_source_json_str"]) {
    fs.writeFileSync(
      "users/" + userId + "/profile_world/avatar_source.json",
      value2(req.body["avatar_source_json_str"]),
    );
  }
  ...
  if (userMeta["is_image_locked"] != true) {
    if (req.files && req.files["profile_image"]) {
      ...
      userMeta["profile_image_url"] =
        "https://bwsecondary.ddns.net:8080/images/profiles/" + userId + ".jpg";
...
async function current_user_profile_world(req: any, res: any) {
  ...
  let src = fs.readFileSync("users/" + userId + "/profile_world/source.json", {
    encoding: "utf8",
  });
  let meta = JSON.parse(
    fs.readFileSync("users/" + userId + "/profile_world/metadata.json", {
  ...
  if (fs.existsSync("users/" + userId + "/profile_world/avatar_source.json")) {
    meta["avatar_source_json_str"] = fs.readFileSync(
      "users/" + userId + "/profile_world/avatar_source.json",
      { encoding: "utf8" },
    );
  }
...
async function current_user_worlds(req: any, res: any) {
  ...
  console.log(
    "User " +
      user.id +
      " owned worlds: " +
      util.inspect(ownedWorlds, { colors: true }),
  );
  ...
  for (const id of ownedWorlds) {
    try {
      ...
      if (
        retrievedWorld !== null &&
        parseInt(retrievedWorld.author_id) === user.id
      ) {
        if (is_published == "yes") {
...
async function current_user_worlds_for_teleport(req: any, res: any) {
  ...
  for (const id of ownedWorlds) {
    try {
      ...
      console.debug(e);
      console.error(
        "could not retrieve worlds for user " + valid.user.id + "!",
      );
      res.status(200).json({
        error: 404,
        error_msg: "Could not load your worlds.",
...
function follow(req: any, res: any) {
  ...
  let user = JSON.parse(
    fs.readFileSync("users/" + userId + "/followed_users.json", {
      encoding: "utf8",
    }),
  );
  let target: any;
  ...
  fs.writeFile(
    "users/" + userId + "/followed_users.json",
    JSON.stringify(user),
    function (err: any) {
      if (err) console.error(err);
      fs.writeFile(
...
export function run(app: any) {
  ...
  app.get(
    "/api/v1/current_user/worlds_for_teleport",
    current_user_worlds_for_teleport,
  );
  app.get("/api/v1/current_user/profile_world", current_user_profile_world);
  app.put(
    "/api/v1/current_user/profile_world",
    save_current_user_profile_world,
  );
  ...
  app.get(
    ...
    async function (req: any, res: any) {
      ...
      if ((await user.exists()) === false) {
        res
          .status(404)
          .json({ error: "404", error_msg: "user does not exists" });
        return;
      }

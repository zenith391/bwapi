...
async function account_login(req, res) {
    ...
    if (fs.existsSync(ACCOUNTS_PATH + username + ".json")) {
        const accountData = JSON.parse(fs.readFileSync(ACCOUNTS_PATH + username + ".json", { "encoding": "utf8" }));
        ...
        bcrypt.compare(req.body.password, "$2a" + accountData.password.substring(3), async function (err, result) {

...
var LinkType;
(function (LinkType) {
    LinkType[LinkType["Wool"] = 1024] = "Wool";
    LinkType[LinkType["iOS"] = 768] = "iOS";
})(LinkType || (LinkType = {}));
;
...
async function save_current_user_profile_world(req, res) {
    ...
    if (!fs.existsSync("users/" + userId + "/profile_world")) {
        ...
        fs.writeFileSync("users/" + userId + "/profile_world/metadata.json", JSON.stringify({
...
async function current_user_profile_world(req, res) {
    ...
    meta = global.processUserWorld(meta);
...
async function current_user_worlds(req, res) {
    ...
    for (const id of ownedWorlds) {
        try {
            let w = await global.fullWorldSync(id, true);
            if (is_published == "yes") {
...
async function current_user_worlds_for_teleport(req, res) {
    ...
    for (const id of ownedWorlds) {
        try {
            let world = await global.fullWorldSync(id, true);
...
export function run(app) {
    ...
    app.get("/api/v1/users/:id/liked_worlds", async function (req, res) {
        ...
        for (let world of worlds) {
            if (world != null) {
                const fWorld = await global.fullWorldSync(world, true);
                if (fWorld == null)
                    continue;
